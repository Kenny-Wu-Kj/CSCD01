...
async function main() {
    ...
    console.log("Poll a single run, status=pending", 
    await client.runs.get(thread.thread_id, run.run_id));

    // We can list events for the run
    console.log("List all events for the run", await client.runs.listEvents(thread.thread_id, run.run_id));
    ...
    console.log("Final run status", await client.runs.get(thread.thread_id, run.run_id))
